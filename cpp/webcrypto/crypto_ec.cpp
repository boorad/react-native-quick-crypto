...
EVPKeyCtxPointer EcKeyPairGen::Setup() {
  EVPKeyCtxPointer key_ctx;
  switch (this->curve_nid) {
    ...
    case EVP_PKEY_X25519:
      // Fall through
    case EVP_PKEY_X448:
      key_ctx.reset(EVP_PKEY_CTX_new_id(this->curve_nid, nullptr));
      break;
    default: {
      ...
      if (!param_ctx ||
          ...
          EVP_PKEY_CTX_set_ec_paramgen_curve_nid(
              param_ctx.get(), this->curve_nid) <= 0 ||
          EVP_PKEY_CTX_set_ec_param_enc(
              param_ctx.get(), this->param_encoding) <= 0 ||
...
void EcKeyPairGen::PrepareConfig(jsi::Runtime &rt,
                                const jsi::Value *args) {
  // curve name
  std::string curveName = args[1].asString(rt).utf8(rt);
  this->curve_nid = GetCurveFromName(curveName.c_str());
